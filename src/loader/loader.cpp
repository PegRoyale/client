...
void loader::load(const char* bin_name)
{
	...
	entry_point_ = (source_nt_headers->OptionalHeader.ImageBase + source_nt_headers->OptionalHeader.AddressOfEntryPoint);
...
std::uint32_t loader::entry_point()
{
	return entry_point_;
}
...
{
	SetDllDirectoryA("data/bins");
	loader::load("data/data.dat");
	LoadLibraryA("pegroyale.dll");
	return loader::run(loader::entry_point());
}
